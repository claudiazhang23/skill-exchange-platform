...
function renderHome() {
  ...
  const tomatoLevel = (skill) => skill.level === "进阶" ? 5 : skill.level === "熟练" ? 4 : 2;
  const tomatoDots = (skill) => `<span class="tomato-dots" aria-label="${escapeHtml(skill.level)}">${Array.from({length:5}, (_, index) => `<i class="tomato-dot ${index < tomatoLevel(skill) ? "on" : ""}"></i>`).join("")}</span>`;
  const skillRows = (skills, type) => skills.slice(0, 3).map((skill) => `<div class="swap-skill-row"><span class="swap-skill-icon"><i data-lucide="${type === "teach" ? "camera" : "pencil"}"></i></span><strong>${escapeHtml(skill.name)}</strong>${tomatoDots(skill)}<button class="icon-button" type="button" data-action="open-skill-modal" data-skill-id="${skill.id}" aria-label="编辑 ${escapeHtml(skill.name)}" title="编辑"><i data-lucide="x"></i></button></div>`).join("");
  const proficiency = [
    ["刚开始", "正在探索，刚接触"], ["有点熟", "了解一些基础"], ["比较熟", "可以完成一般任务"], ["很熟练", "可以独立完成"], ["可以教别人", "可以帮助别人"],
  ];
  return `<section class="page home-page reference-home">
    <section class="reference-hero"><div class="reference-buddy"><span class="buddy-note">小技能<br />大世界</span><div class="tomato-buddy" aria-label="Tomato Buddy"></div></div><div class="reference-copy"><span class="brand-eyebrow">SKILL SWAP · 技能互换</span><h1>今天，你想交换什么？</h1><p>在这里，分享你会的，学习你想学的。<br />用技能连接有趣的人，一起成长。</p><button class="primary-button hero-start" type="button" data-view="matches">开始寻找伙伴 <i data-lucide="arrow-right"></i></button><div class="reference-stats"><span><strong>12,480+</strong><small>用户加入</small></span><span><strong>8,320+</strong><small>成功匹配</small></span><span><strong>200+</strong><small>城市的番茄们</small></span></div></div><section class="swap-panels"><div class="swap-panel teach"><div class="swap-panel-head"><i data-lucide="badge-check"></i><div><h2>我能分享的</h2><p>你会什么？选择你擅长的技能</p></div></div><div class="swap-skill-list">${skillRows(user.canTeach, "teach")}</div><button class="add-skill-button" type="button" data-action="open-skill-modal" data-skill-type="teach"><i data-lucide="plus"></i>添加技能</button></div><div class="swap-connector"><i data-lucide="arrow-left-right"></i><span>交换技能<br />交换可能</span></div><div class="swap-panel learn"><div class="swap-panel-head"><i data-lucide="graduation-cap"></i><div><h2>我想学习的</h2><p>你想学什么？选择你的目标</p></div></div><div class="swap-skill-list">${skillRows(user.wantToLearn, "learn")}</div><button class="add-skill-button" type="button" data-action="open-skill-modal" data-skill-type="learn"><i data-lucide="plus"></i>添加技能</button></div></section></section>
    <section class="proficiency-section"><div class="proficiency-intro"><span class="section-kicker"><i data-lucide="star"></i>Skill proficiency</span><h2>番茄熟悉度系统</h2><p>用可爱的番茄，标记你的技能熟练度，让沟通更简单有趣。</p><button class="secondary-button" type="button" data-view="profile">了解更多 <i data-lucide="arrow-right"></i></button></div><div class="proficiency-track">${proficiency.map((item, index) => `<div class="proficiency-step"><div class="tomato-level level-${index+1}" role="img" aria-label="${item[0]}"></div><strong>${item[0]}</strong><span>${item[1]}</span>${index < proficiency.length - 1 ? `<i class="proficiency-arrow" data-lucide="chevron-right"></i>` : ""}</div>`).join("")}</div></section>
    <section class="home-people reference-match-section"><div class="section-title"><div><span class="section-kicker"><i data-lucide="heart"></i>今日匹配推荐</span><h2>这些有趣的番茄们，正在寻找技能交换伙伴！</h2></div><button class="text-button" type="button" data-view="discover">查看更多 <i data-lucide="arrow-right"></i></button></div><div class="home-people-grid">${matches.map((match) => renderMatchCard(match, true)).join("")}</div></section>
    <section class="skill-folder-section"><div class="section-title"><div><span class="section-kicker"><i data-lucide="book-open"></i>Skill passport</span><h2>我的技能卡组</h2><p>管理你的技能，展示独特的你。</p></div><button class="primary-button" type="button" data-view="profile">编辑我的技能 <i data-lucide="arrow-right"></i></button></div><div class="skill-folder-grid">${user.canTeach.slice(0,5).map((skill) => `<article class="skill-folder-card"><span class="folder-icon"><i data-lucide="camera"></i></span><strong>${escapeHtml(skill.name)}</strong><span class="folder-level">${escapeHtml(skill.level)}</span><p>${escapeHtml(skill.description || "用技能让生活更多可能")}</p><div>${tomatoDots(skill)}</div></article>`).join("")}<button class="skill-folder-card add-folder" type="button" data-action="open-skill-modal" data-skill-type="teach"><i data-lucide="plus"></i><span>添加技能</span></button></div></section>
  </section>`;
}
